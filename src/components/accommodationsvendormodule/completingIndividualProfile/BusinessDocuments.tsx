'use client';

import '../../../app/flags.css';
import React, { useRef, useState } from 'react';
import { Form, Button } from 'react-bootstrap';
import DatePickers from '@/components/DatePickers';
import Link from 'next/link';

interface Props {
    nextStep: () => void;
}

const BusinessDocuments : React.FC<Props> = ({ nextStep }) => {
    const inputRefUserId = useRef<HTMLInputElement>(null);
    const inputRefDocument = useRef<HTMLInputElement>(null);
    const inputRefRegistration = useRef<HTMLInputElement>(null);
    const inputRefOperating = useRef<HTMLInputElement>(null);

    const [userIdFiles, setUserIdFiles] = useState<File[]>([]);
    const [documentFiles, setDocumentFiles] = useState<File[]>([]);
    const [registrationFiles, setRegistrationFiles] = useState<File[]>([]);
    const [operatingFiles, setOperatingFiles] = useState<File[]>([]);

    

    const handleDrop = (
        e: React.DragEvent,
        target: 'userId' | 'document' | 'registration' | 'operating'
    ) => {
        e.preventDefault();
        const files = Array.from(e.dataTransfer.files);
        if (target === 'userId') {
            setUserIdFiles((prev) => [...prev, ...files]);
        } else if (target === 'document') {
            setDocumentFiles((prev) => [...prev, ...files]);
        } else if (target === 'registration') {
            setRegistrationFiles((prev) => [...prev, ...files]);
        } else if (target === 'operating') {
            setOperatingFiles((prev) => [...prev, ...files]);
        }
    };

    const handleChange = (
        e: React.ChangeEvent<HTMLInputElement>,
        target: 'userId' | 'document' | 'registration' | 'operating'
    ) => {
        const files = e.target.files ? Array.from(e.target.files) : [];
        if (target === 'userId') {
            setUserIdFiles((prev) => [...prev, ...files]);
        } else if (target === 'document') {
            setDocumentFiles((prev) => [...prev, ...files]);
        } else if (target === 'registration') {
            setRegistrationFiles((prev) => [...prev, ...files]);
        } else if (target === 'operating') {
            setOperatingFiles((prev) => [...prev, ...files]);
        }
    };

    const triggerUpload = (target: 'userId' | 'document' | 'registration' | 'operating') => {
        if (target === 'userId') inputRefUserId.current?.click();
        else if (target === 'document') inputRefDocument.current?.click();
        else if (target === 'registration') inputRefRegistration.current?.click();
        else if (target === 'operating') inputRefOperating.current?.click();
    };

    const renderFileList = (files: File[]) => (
        <ul className="list-unstyled small mt-2 mb-0">
            {files.map((file, idx) => (
                <li key={idx}>{file.name}</li>
            ))}
        </ul>
    );


    return (

        <div className="card-box">
            <Form>
                <div className='row'>
                    <div className='col-md-6'>
                        <div className="form-group mb-24 ">
                            <Form.Label>Upload Business Registration</Form.Label>
                            <div onClick={() => triggerUpload('userId')}
                                onDrop={(e) => handleDrop(e, 'userId')}
                                onDragOver={(e) => e.preventDefault()}
                                className="border-upload text-center d-flex align-items-center justify-content-center"
                                style={{ height: '160px', width: '100%', cursor: 'pointer' }}>
                                <div className="text-muted small">
                                    <div className="fs-4">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 20.0001C5.45 20.0001 4.97933 19.8044 4.588 19.4131C4.19667 19.0217 4.00067 18.5507 4 18.0001V16.0001C4 15.7167 4.096 15.4794 4.288 15.2881C4.48 15.0967 4.71733 15.0007 5 15.0001C5.28267 14.9994 5.52033 15.0954 5.713 15.2881C5.90567 15.4807 6.00133 15.7181 6 16.0001V18.0001H18V16.0001C18 15.7167 18.096 15.4794 18.288 15.2881C18.48 15.0967 18.7173 15.0007 19 15.0001C19.2827 14.9994 19.5203 15.0954 19.713 15.2881C19.9057 15.4807 20.0013 15.7181 20 16.0001V18.0001C20 18.5501 19.8043 19.0211 19.413 19.4131C19.0217 19.8051 18.5507 20.0007 18 20.0001H6ZM11 7.85006L9.125 9.72506C8.925 9.92506 8.68767 10.0211 8.413 10.0131C8.13833 10.0051 7.90067 9.90073 7.7 9.70006C7.51667 9.50006 7.42067 9.26673 7.412 9.00006C7.40333 8.7334 7.49933 8.50006 7.7 8.30006L11.3 4.70006C11.4 4.60006 11.5083 4.5294 11.625 4.48806C11.7417 4.44673 11.8667 4.42573 12 4.42506C12.1333 4.4244 12.2583 4.4454 12.375 4.48806C12.4917 4.53073 12.6 4.6014 12.7 4.70006L16.3 8.30006C16.5 8.50006 16.596 8.7334 16.588 9.00006C16.58 9.26673 16.484 9.50006 16.3 9.70006C16.1 9.90006 15.8627 10.0044 15.588 10.0131C15.3133 10.0217 15.0757 9.92573 14.875 9.72506L13 7.85006V15.0001C13 15.2834 12.904 15.5211 12.712 15.7131C12.52 15.9051 12.2827 16.0007 12 16.0001C11.7173 15.9994 11.48 15.9034 11.288 15.7121C11.096 15.5207 11 15.2834 11 15.0001V7.85006Z" fill="#838383" /></svg>
                                    </div>
                                    {userIdFiles.length > 0
                                        ? renderFileList(userIdFiles)
                                        : 'Click To Upload Media File Or Drag And Drop Your Files Here...'}
                                </div>
                                <Form.Control
                                    type="file"
                                    multiple
                                    onChange={(e) => handleChange(e, 'userId')}
                                    ref={inputRefUserId}
                                    className="d-none" />
                            </div>
                        </div>
                        <div className="form-group mb-24">
                            <Form.Label>Business Registration Number</Form.Label>
                            <Form.Control type="text" placeholder='1234567980' />
                        </div>

                        <div className="form-group mb-24 ">
                            <Form.Label>Upload Tax Registration</Form.Label>
                            <div
                                onClick={() => triggerUpload('registration')}
                                onDrop={(e) => handleDrop(e, 'registration')}
                                onDragOver={(e) => e.preventDefault()}
                                className="border-upload text-center d-flex align-items-center justify-content-center"
                                style={{ height: '160px', width: '100%', cursor: 'pointer' }}
                            >
                                <div className="text-muted small">
                                    <div className="fs-4">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 20.0001C5.45 20.0001 4.97933 19.8044 4.588 19.4131C4.19667 19.0217 4.00067 18.5507 4 18.0001V16.0001C4 15.7167 4.096 15.4794 4.288 15.2881C4.48 15.0967 4.71733 15.0007 5 15.0001C5.28267 14.9994 5.52033 15.0954 5.713 15.2881C5.90567 15.4807 6.00133 15.7181 6 16.0001V18.0001H18V16.0001C18 15.7167 18.096 15.4794 18.288 15.2881C18.48 15.0967 18.7173 15.0007 19 15.0001C19.2827 14.9994 19.5203 15.0954 19.713 15.2881C19.9057 15.4807 20.0013 15.7181 20 16.0001V18.0001C20 18.5501 19.8043 19.0211 19.413 19.4131C19.0217 19.8051 18.5507 20.0007 18 20.0001H6ZM11 7.85006L9.125 9.72506C8.925 9.92506 8.68767 10.0211 8.413 10.0131C8.13833 10.0051 7.90067 9.90073 7.7 9.70006C7.51667 9.50006 7.42067 9.26673 7.412 9.00006C7.40333 8.7334 7.49933 8.50006 7.7 8.30006L11.3 4.70006C11.4 4.60006 11.5083 4.5294 11.625 4.48806C11.7417 4.44673 11.8667 4.42573 12 4.42506C12.1333 4.4244 12.2583 4.4454 12.375 4.48806C12.4917 4.53073 12.6 4.6014 12.7 4.70006L16.3 8.30006C16.5 8.50006 16.596 8.7334 16.588 9.00006C16.58 9.26673 16.484 9.50006 16.3 9.70006C16.1 9.90006 15.8627 10.0044 15.588 10.0131C15.3133 10.0217 15.0757 9.92573 14.875 9.72506L13 7.85006V15.0001C13 15.2834 12.904 15.5211 12.712 15.7131C12.52 15.9051 12.2827 16.0007 12 16.0001C11.7173 15.9994 11.48 15.9034 11.288 15.7121C11.096 15.5207 11 15.2834 11 15.0001V7.85006Z" fill="#838383" /></svg>
                                    </div>
                                    {registrationFiles.length > 0
                                        ? renderFileList(registrationFiles)
                                        : 'Click To Upload Media File Or Drag And Drop Your Files Here...'}
                                </div>
                                <Form.Control
                                    type="file"
                                    multiple
                                    onChange={(e) => handleChange(e, 'registration')}
                                    ref={inputRefRegistration}
                                    className="d-none"
                                />
                            </div>
                        </div>

                        <div className='row'>
                            <div className='col-md-6'>
                                <div className="form-group mb-24 mb-md-0">
                                    <Form.Label>Tax registration ID</Form.Label>
                                    <Form.Control type="text" placeholder='1234567980' />
                                </div>
                            </div>
                            <div className='col-md-6'>
                                <div className="form-group mb-1 mb-md-0">
                                    <Form.Label>Registered On</Form.Label>
                                    <DatePickers />
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col-md-6'>
                        <div className="form-group mb-24">
                            <Form.Label>Upload Driving Licence</Form.Label>
                            <div
                                onClick={() => triggerUpload('document')}
                                onDrop={(e) => handleDrop(e, 'document')}
                                onDragOver={(e) => e.preventDefault()}
                                className="border-upload text-center d-flex align-items-center justify-content-center"
                                style={{ height: '160px', width: '100%', cursor: 'pointer' }}
                            >
                                <div className="text-muted small">
                                    <div className="fs-4">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 20.0001C5.45 20.0001 4.97933 19.8044 4.588 19.4131C4.19667 19.0217 4.00067 18.5507 4 18.0001V16.0001C4 15.7167 4.096 15.4794 4.288 15.2881C4.48 15.0967 4.71733 15.0007 5 15.0001C5.28267 14.9994 5.52033 15.0954 5.713 15.2881C5.90567 15.4807 6.00133 15.7181 6 16.0001V18.0001H18V16.0001C18 15.7167 18.096 15.4794 18.288 15.2881C18.48 15.0967 18.7173 15.0007 19 15.0001C19.2827 14.9994 19.5203 15.0954 19.713 15.2881C19.9057 15.4807 20.0013 15.7181 20 16.0001V18.0001C20 18.5501 19.8043 19.0211 19.413 19.4131C19.0217 19.8051 18.5507 20.0007 18 20.0001H6ZM11 7.85006L9.125 9.72506C8.925 9.92506 8.68767 10.0211 8.413 10.0131C8.13833 10.0051 7.90067 9.90073 7.7 9.70006C7.51667 9.50006 7.42067 9.26673 7.412 9.00006C7.40333 8.7334 7.49933 8.50006 7.7 8.30006L11.3 4.70006C11.4 4.60006 11.5083 4.5294 11.625 4.48806C11.7417 4.44673 11.8667 4.42573 12 4.42506C12.1333 4.4244 12.2583 4.4454 12.375 4.48806C12.4917 4.53073 12.6 4.6014 12.7 4.70006L16.3 8.30006C16.5 8.50006 16.596 8.7334 16.588 9.00006C16.58 9.26673 16.484 9.50006 16.3 9.70006C16.1 9.90006 15.8627 10.0044 15.588 10.0131C15.3133 10.0217 15.0757 9.92573 14.875 9.72506L13 7.85006V15.0001C13 15.2834 12.904 15.5211 12.712 15.7131C12.52 15.9051 12.2827 16.0007 12 16.0001C11.7173 15.9994 11.48 15.9034 11.288 15.7121C11.096 15.5207 11 15.2834 11 15.0001V7.85006Z" fill="#838383" /></svg>
                                    </div>
                                    {documentFiles.length > 0
                                        ? renderFileList(documentFiles)
                                        : 'Click To Upload Media File Or Drag And Drop Your Files Here...'}
                                </div>
                                <Form.Control
                                    type="file"
                                    multiple
                                    onChange={(e) => handleChange(e, 'document')}
                                    ref={inputRefDocument}
                                    className="d-none"
                                />
                            </div>
                        </div>
                        <div className="form-group mb-24">
                            <Form.Label>Business Pin</Form.Label>
                            <div className='position-relative'>
                                <Link href="" className='location-btn  d-flex align-items-center gap-1'> 
                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.37331 13.88C8.26451 13.9581 8.13393 14.0002 7.99998 14.0002C7.86602 14.0002 7.73544 13.9581 7.62664 13.88C4.40731 11.5853 0.990642 6.86533 4.44464 3.45467C5.39287 2.5219 6.66987 1.99942 7.99998 2C9.33331 2 10.6126 2.52333 11.5553 3.454C15.0093 6.86467 11.5926 11.584 8.37331 13.88Z" stroke="#262626" strokeWidth="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M7.99996 7.99992C8.35358 7.99992 8.69272 7.85944 8.94277 7.60939C9.19282 7.35935 9.33329 7.02021 9.33329 6.66659C9.33329 6.31296 9.19282 5.97383 8.94277 5.72378C8.69272 5.47373 8.35358 5.33325 7.99996 5.33325C7.64634 5.33325 7.3072 5.47373 7.05715 5.72378C6.8071 5.97383 6.66663 6.31296 6.66663 6.66659C6.66663 7.02021 6.8071 7.35935 7.05715 7.60939C7.3072 7.85944 7.64634 7.99992 7.99996 7.99992Z" stroke="#262626" strokeWidth="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                                    Location
                                </Link>
                                <Form.Control type="text" placeholder='pin location' />
                            </div>
                        </div>

                        <div className="form-group mb-24">
                            <Form.Label>Upload Operating Permit</Form.Label>
                            <div
                                onClick={() => triggerUpload('operating')}
                                onDrop={(e) => handleDrop(e, 'operating')}
                                onDragOver={(e) => e.preventDefault()}
                                className="border-upload text-center d-flex align-items-center justify-content-center"
                                style={{ height: '160px', width: '100%', cursor: 'pointer' }}
                            >
                                <div className="text-muted small">
                                    <div className="fs-4">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 20.0001C5.45 20.0001 4.97933 19.8044 4.588 19.4131C4.19667 19.0217 4.00067 18.5507 4 18.0001V16.0001C4 15.7167 4.096 15.4794 4.288 15.2881C4.48 15.0967 4.71733 15.0007 5 15.0001C5.28267 14.9994 5.52033 15.0954 5.713 15.2881C5.90567 15.4807 6.00133 15.7181 6 16.0001V18.0001H18V16.0001C18 15.7167 18.096 15.4794 18.288 15.2881C18.48 15.0967 18.7173 15.0007 19 15.0001C19.2827 14.9994 19.5203 15.0954 19.713 15.2881C19.9057 15.4807 20.0013 15.7181 20 16.0001V18.0001C20 18.5501 19.8043 19.0211 19.413 19.4131C19.0217 19.8051 18.5507 20.0007 18 20.0001H6ZM11 7.85006L9.125 9.72506C8.925 9.92506 8.68767 10.0211 8.413 10.0131C8.13833 10.0051 7.90067 9.90073 7.7 9.70006C7.51667 9.50006 7.42067 9.26673 7.412 9.00006C7.40333 8.7334 7.49933 8.50006 7.7 8.30006L11.3 4.70006C11.4 4.60006 11.5083 4.5294 11.625 4.48806C11.7417 4.44673 11.8667 4.42573 12 4.42506C12.1333 4.4244 12.2583 4.4454 12.375 4.48806C12.4917 4.53073 12.6 4.6014 12.7 4.70006L16.3 8.30006C16.5 8.50006 16.596 8.7334 16.588 9.00006C16.58 9.26673 16.484 9.50006 16.3 9.70006C16.1 9.90006 15.8627 10.0044 15.588 10.0131C15.3133 10.0217 15.0757 9.92573 14.875 9.72506L13 7.85006V15.0001C13 15.2834 12.904 15.5211 12.712 15.7131C12.52 15.9051 12.2827 16.0007 12 16.0001C11.7173 15.9994 11.48 15.9034 11.288 15.7121C11.096 15.5207 11 15.2834 11 15.0001V7.85006Z" fill="#838383" /></svg>
                                    </div>
                                    {operatingFiles.length > 0
                                        ? renderFileList(operatingFiles)
                                        : 'Click To Upload Media File Or Drag And Drop Your Files Here...'}
                                </div>
                                <Form.Control
                                    type="file"
                                    multiple
                                    onChange={(e) => handleChange(e, 'operating')}
                                    ref={inputRefOperating}
                                    className="d-none"
                                />
                            </div>
                        </div>


                        <div className='row'>
                            <div className='col-md-6'>
                                <div className="form-group mb-24 mb-md-0">
                                    <Form.Label>Permit Id</Form.Label>
                                    <Form.Control type="text" placeholder='1234567980' />
                                </div>
                            </div>
                            <div className='col-md-6'>
                                <div className="form-group mb-24 mb-md-0">
                                    <Form.Label>Valid Through</Form.Label>
                                    <DatePickers />
                                </div>
                            </div>
                        </div>

                    </div>
                </div>
                <div className="footer-step-form d-flex justify-content-end">
                    <Button variant="primary" onClick={nextStep} className="btn btn-next-box">
                        Save & Next
                    </Button>
                </div>
            </Form>
        </div>
  )
}

export default BusinessDocuments